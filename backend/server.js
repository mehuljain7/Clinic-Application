...
app.post("/api/resend-verification", async (req, res) => {
  ...
  try {
    ...
    await sendEmail(
      ...
      `<p>Click <a href="${verifyUrl}" target="_blank">here</a> to verify your email.</p>`
    );
...
app.post("/reset/:token", async (req, res) => {
  ...
  try {
    ...
    res.send(
      `<h2>Password reset successful ✅</h2><p>You can now log in with your new password.</p>`
    );
  } catch (err) {
    console.error(err);
    res.status(500).send("Server error");
...
app.post("/api/saveDiagnosis", async (req, res) => {
  ...
  try {
    if (!userEmail || !inputs || !results)
      return res
        .status(400)
        .json({ error: "Missing required fields: userEmail, inputs, results" });
